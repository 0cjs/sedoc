...
Interrupt vectors are given as the address of four sequential words
(aligned to an eight byte boundary) for channels 0, 1, 2 and 3
respectively. This is a single address always written to channel 0.
(This expects you're using interrupt mode 2 on the Z80.)

    Bits  Setting
    ───────────────────────────────────────────────────────────────────
    b7-3  5 MSbits of interrupt vector base (8-byte aligned)
    b2-1  ignored (internally, channel identifier 0-3)
     b0   =0 for Interrupt Vector Word
...
The control word is a byte with the following format:
    ...
    ──────────────────────────────────────────────────────────
     ...
     b1  Reset             continue operation   software reset
     b0  Word Type         —                    chan.control

Notes:
- † `b5` and `b3` apply in timer mode only.
- `b2=1`: the next write must be an 8-bit time constant to load into the
  time constant register for that channel. (0 = time constant 256.)
- `b1=1`: Stops channel immediately. Restart when next time constant loaded.
