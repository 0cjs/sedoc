...
The following files are involved, in this order:
- `HASH-env-manifest.nix`: A Nix expression describing the environment,
  created by Nix when e.g. `nix-env --install` is run. This contains a list
  of sets, each set describing a Nix package included in the environment,
  including the package name, system, "output" path under `/nix/store/` and
  package metadata.
- `HASH-user-environment.drv`: A [derivation] description compiled from the
  expression above.
- `HASH-user-environment/`: the package built from the above derivation. At
  the top level will be a `manifest.nix` symlink to the original
  `HASH-env-manifest.nix` expression above.
