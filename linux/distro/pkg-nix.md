...
Derivations are generated by evaluating _expressions_ in the pure, lazy,
functional [Nix expression language][nix expr]. Outputs, derivations
(`.drv` files) and expressions (`.nix` files) are all stored under
`/nix/store/`, named by an SHA256 of the inputs followed by a
